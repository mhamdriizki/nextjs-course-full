...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen p-8 lg:p-24 bg-gray-50 dark:bg-slate-950">
      {/* Chip untuk mengetahui apakah user sudah login atau belum */}
      <div className="mb-4 flext items-center gap-4">
        {isPending ? (
          <Badge variant="outline">Memuat sesi . . .</Badge>
        ) : session ? (
          <Badge variant="default" className="bg-green-600">Login sebagai: {session.user.name ||
          session.user.email}</Badge>
        ) : (
