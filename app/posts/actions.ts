...
const DEMO_AUTHOR_EMAIL = "rizki@example.com";

// Mutasi (termasuk find-or-create) sengaja ditaruh di Server Action,
// bukan di render Server Component — project ini pakai Cache Components,
// yang melarang side effect semacam ini terjadi selama render.
async function getOrCreateDemoAuthor() {
  const existing = await db.user.findFirst({ where: { email: DEMO_AUTHOR_EMAIL } });
  if (existing) return existing;
  return db.user.create({
    data: { email: DEMO_AUTHOR_EMAIL, name: "Muhammad Rizki", role: "AUTHOR" },
  });
}
...
export async function createPostAction(formData: FormData) {
  ...
  await createPost({ title, slug, excerpt, content: content || undefined, authorId: author.id });
